...
/**
 * A rare, terrain-checked Overworld structure assembled from the same 18
 * templates as the manual Maraw‘Thar arena deployment command.
 */
public final class MarawTharArenaStructure extends Structure {
    public static final Codec<MarawTharArenaStructure> CODEC =
            simpleCodec(MarawTharArenaStructure::new);

    private static final int MAX_RELIEF = 8;
    private static final int MAX_SURFACE_DEPTH = 3;

    public MarawTharArenaStructure(StructureSettings settings) {
        super(settings);
    }
    ...
    protected Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        ...
        int radius = ArenaDefinitions.MARAWTHAR.size().getX() / 2;
        ...
        return Optional.of(new GenerationStub(floorAnchor, builder -> {
            for (int index = 0; index < definition.tileCount(); index++) {
                ...
                builder.addPiece(new MarawTharArenaPiece(
    ...
    public StructureType<?> type() {
        return ModWorldgen.MARAWTHAR_ARENA.get();
    }
}
