...
/**
 * A rare, terrain-checked Overworld structure assembled from the same eight
 * templates as the manual B5 arena deployment command.
 */
public final class B5ArenaStructure extends Structure {
    public static final Codec<B5ArenaStructure> CODEC =
            simpleCodec(B5ArenaStructure::new);

    private static final int MAX_RELIEF = 8;
    private static final int MAX_SURFACE_DEPTH = 3;

    public B5ArenaStructure(StructureSettings settings) {
        super(settings);
    }
    ...
    protected Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        ...
        int radius = ArenaDefinitions.B5.size().getX() / 2;
        ...
        return Optional.of(new GenerationStub(floorAnchor, builder -> {
            for (int index = 0; index < definition.tileCount(); index++) {
                ...
                builder.addPiece(new B5ArenaPiece(
    ...
    public StructureType<?> type() {
        return ModWorldgen.B5_ARENA.get();
    }
}
