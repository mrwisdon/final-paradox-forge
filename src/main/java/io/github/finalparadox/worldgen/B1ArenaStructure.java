 ...
 * as the manual B1 arena deployment command.
 */
public final class B1ArenaStructure extends Structure {
    public static final Codec<B1ArenaStructure> CODEC =
            simpleCodec(B1ArenaStructure::new);

    private static final int MAX_RELIEF = 8;
    private static final int MAX_SURFACE_DEPTH = 3;

    public B1ArenaStructure(StructureSettings settings) {
        super(settings);
    }
    ...
    protected Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        ...
        int radius = ArenaDefinitions.B1.size().getX() / 2;
        ...
        return Optional.of(new GenerationStub(floorAnchor, builder -> {
            for (int index = 0; index < definition.tileCount(); index++) {
                ...
                builder.addPiece(new B1ArenaPiece(
    ...
    public StructureType<?> type() {
        return ModWorldgen.B1_ARENA.get();
    }
}
