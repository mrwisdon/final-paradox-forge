...
状态：**已在 `fix/terrastalker-dismount-sync` 分支实现**（2026-08-04）。
`B8EncounterController.acechadorRound()` 已接入独立敌对实体，轮次等待与清理也已完成。
本文档保留已提取的权威证据，后续维护无需重新解包数据包。

> 2026-08-04 用户确认的 B8 实现边界：敌方大地漫游器只有机炮，不使用导弹。
> 下文导弹段保留为原始提取证据，但不属于本次 B8 敌车实现。
> 敌方机炮命中载具骑手时优先扣除我方大地漫游器 1 点能量，不伤害骑手；
> 玩家未乘坐大地漫游器时才直接承受机炮伤害。
